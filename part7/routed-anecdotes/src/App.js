...
import { Routes, Route, Link, Navigate, useMatch, redirect, useNavigate } from "react-router-dom"
...
const SingleAnecdote = ({ anecdote }) => {
  return (
    <div>
      ...
      <p>For more information checkout {anecdote.info}</p>
    </div>
  )
}
...
const App = () => {
  ...
  const matchedAnecdote = match ? anecdotes.find(a => a.id === Number(match.params.id)) : null;
  ...
  return (
    <div>
      ...
      <div>
        <Routes>
          <Route path="/anecdotes/:id" element={<SingleAnecdote anecdote={matchedAnecdote} />}></Route>
          <Route path="/" element={<AnecdoteList anecdotes={anecdotes} notification={notification} />}></Route>
